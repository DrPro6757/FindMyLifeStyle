...
const MyFollower = (props) => {
    ...
    const [myFollowersList, setMyFollowersList] = useState(null);
...
const myFollowerListData = async()=>{
        ...
        await firestore().collection('users').doc(auth().currentUser?.uid).get()
            .then(
                querySnapshot => {
                    ...
                    console.log("my followers data == " + myFollowers);
                    myFollowers.map(item1 => {
                        if (item1 == myFollowers) {
                            console.log('all foolowers ;; ' + item1)
                        } else {
                            // console.log('all foolowers ;; ' + item1)
                            // firestore()
                            //     .collection('users')
                            ...
                            //             tempMyFollowerList.push(item1);
                            //             console.log('users ::' + documentSnapshot._data.name, documentSnapshot._data.age)
                            //             setMyFollowersList(item1)
                            //             console.log('final followers list :: ' + myFollowersList)
                            //         });
    ...
    return (
        <View>
            {
                    myFollowersList !== null ?
                    myFollowersList.map((item)=>{
                        return(
