...
const YourPosts = () => {
  ...
  const [caption, setCaption] = useState(route.params.data.caption == '' ? '' : route.params.data.caption);
...
const openCamera = async () => {
  ...
  const openGallery = async (type) => {
    ...
    const getAllUserPostsData=()=>{
        axios.get('http://10.0.2.2:8000/api/userposts/'+route.params.data._id)
        .then((res)=>{
    ...
    return (
        ...
        <View style={styles.container}>
          ...
          <View style={{ marginVertical: 20 }}>
            <Text style={{ color: 'white', fontSize: 22, fontWeight: '500' }}>Edit Caption</Text>
